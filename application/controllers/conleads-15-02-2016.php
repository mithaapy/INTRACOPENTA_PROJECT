...
//        $data['data_customers'] = $this->model_customers->viewall();
        $data['leads'] = $this->model_leads->openbidding();
    ...
    public function insert() {
        ...
        /*
         * After insert email well send
         */
        
        $sender_email = 'lead@inta.com';
        ...
        $config['smtp_pass'] = 'cajobportal@123';
        $this->email->set_mailtype("html");
        // Load email library and passing configured values to email library
        $this->load->library('email', $config);

        $subject = $paper_name . " Paper Answerd By " . $name;
        $message = '<h3>Hello Admin </h3>';
        $message.= $name . ' (' . $session_email . ')' . ' has been answar and submited the paper (' . $paper_name . ') on ' . date("Y/m/d") . ' at ' . date("h:i:sa") . '. Please review it.';
        $message.='<p></p>';
        $message.='Thanks & Regrds<br />';
        $message.='Team CA Job Portal';
        $this->email->to($to);
        $this->email->from($sender_email, 'CA Job Portal');

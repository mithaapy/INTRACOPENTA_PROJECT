                    ...
                    <table class='table table-bordered table-striped table-condensed datatables'>
                        <thead>
                            <tr>
								...
								<th style="text-align: center; vertical-align: middle;">Max Discount (%)</th>
                        ...
                        <tbody>
                            <?php foreach ($roles as $row): ?>
                                <tr>
									...
									<td style="text-align: left; vertical-align: middle;"><?php echo $row->maxdiscount ?></td>
                                    <td style="text-align: center; vertical-align: middle;">
                                        ...
                                        <?php if ($row->id > '8'): ?>
                                        <a class='btn btn-warning btn-sm' href='<?php echo base_url() ?>index.php/conroles/delete/<?php echo $row->id ?>' title="Delete" onclick="return confirm('Are you sure want to delete this data?')"><i class='fa fa-trash-o'></i></a>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
...
<script type="text/javascript">
    function action_form(type, id) {
        $('.alert-notif').slideUp();
        $.ajax({
            url: '<?php echo base_url() ?>index.php/conroles/get_form',
            type: 'POST',

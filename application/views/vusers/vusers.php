        ...
        <div class="panel panel-default">
            <div class="panel-body">
                ...
                <div class='table-responsive' id='divgridview'>
                    ...
                    <a class='btn btn-default' href='<?php echo base_url() ?>assets/files/export/default/template_users.xls'><i class='fa fa-download'></i> Download Template</a>
                    <hr/>
                    <table class='table table-bordered table-striped table-condensed datatables'>
                        <thead>
                            <tr>
                                ...
                                <th style="text-align: center; vertical-align: middle;">Country / City</th>
                        ...
                        <tbody>
                            <?php foreach ($users as $row): ?>
                                <tr>
                                    ...
                                    <td style="text-align: left; vertical-align: middle;"><?php echo $row->users_nik ?></td>
                                    <td style="text-align: left; vertical-align: middle;"><?php echo $row->users_firstname . ' ' . $row->users_lastname ?></td>
                                    <td style="text-align: left; vertical-align: middle;"><?php echo $row->users_gender ?></td>
                                    <td style="text-align: left; vertical-align: middle;"><?php echo $row->users_birthdate ?></td>
                                    <td style="text-align: left; vertical-align: middle;"><?php echo $row->users_phone . ' / ' . $row->users_mobile . ' / ' . $row->users_pinbbm . ' / ' . $row->users_email ?></td>
                                    <td style="text-align: left; vertical-align: middle;"><?php echo $row->countries_code.' - '.$row->countries_name . ' / ' . $row->cities_code.' - '.$row->cities_name ?></td>
                                    <td style="text-align: left; vertical-align: middle;"><?php echo $row->roles_name ?></td>
                                    <td style="text-align: center; vertical-align: middle;">
                                        <a class='btn btn-primary btn-sm' href='javascript:;' onclick="action_form('detail', '<?php echo $row->users_id ?>')" title="Detail"><i class='fa fa-search'></i></a>
                                        <a class='btn btn-default btn-sm' href='javascript:;' onclick="action_form('edit', '<?php echo $row->users_id ?>')" title="Edit"><i class='fa fa-edit'></i></a>
                                        <a class='btn btn-warning btn-sm' href='<?php echo base_url() ?>index.php/conusers/delete/<?php echo $row->users_id ?>' title="Delete" onclick="return confirm('Are you sure want to delete this data?')"><i class='fa fa-trash-o'></i></a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
...
<script type="text/javascript">
    function action_form(type, id) {
        $('.alert-notif').slideUp();
        $.ajax({
            url: '<?php echo base_url() ?>index.php/conusers/get_form',
            type: 'POST',

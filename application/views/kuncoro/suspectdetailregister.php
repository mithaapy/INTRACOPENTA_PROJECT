        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    <h1 class='ekunfontslide'>
                        Suspect Detail Registration 
                    </h1>
                    <ol class="breadcrumb ekunfontslide">
                        <li><a href="#"><i class="fa fa-thumb-tack"></i> INTA</a></li>
                        <li>Lead</li>
                        <li>Suspect Detail </li>
                        <li class="active">Suspect Detail Registration </li>
                    </ol>
                </section>
            ...
            <form id="form" class="form-horizontal" name="form" action="../../updateSuspectDetail/<?php echo $mainid;?>/<?php echo $detailid;?>" method="POST" enctype="multipart/form-data">
    ...
    <div class="col-md-12">
        <div class="box box-info">
            <div class="box-header with-border">
                ...
                <div class="box-body">
                    <div class="row">
                        <div class="col-md-12">
                                <div class="form-group">
                                    <label for="input-name" class="col-sm-2 control-label">Segment ID</label>
                                    <div class="col-sm-5">
                                        <!--<input type="text" class="form-control" id="input-segment-id" name="input-segment-id" placeholder="Segment ID" value="<?php if(!empty($suspectdetailreg)) echo $suspectdetailreg[0]['SEGMENT_ID']; ?>">-->

                                        <select class="form-control" id="input-segment-id" name="input-segment-id">
                                            <?php
                                            foreach ($Segmentlist as $key) {
                                                if (!empty($suspectReg)) {
                                                    if ($suspectReg[0]['SEGMENTID']==$key['SEGMENTID']) {
                                                        echo "<option selected='selected' value=".$key['SEGMENTID'].">".$key['INDUSTRY_SEGMENT']."</option>";
                                                    }else{
                                                        echo "<option value=".$key['SEGMENTID'].">".$key['INDUSTRY_SEGMENT']."</option>";
                                                    }
                                                }else{
                                                    echo "<option value=".$key['SEGMENTID'].">".$key['INDUSTRY_SEGMENT']."</option>";
                                                }
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-control" id="select-product-id" name="select-product-id">
                                            <?php
                                            foreach ($productlist as $key) {
                                                if (!empty($suspectReg)) {
                                                    if ($suspectReg[0]['PRODUCTID']==$key['PRODUCTID']) {
                                ...
                                <div class="form-group">
                                    <label for="input-name" class="col-sm-2 control-label">Suspect ID</label>
                                    <div class="col-sm-3">
                                        <input type="text" class="form-control" id="input-suspect-id" name="input-suspect-id" placeholder="Suspect ID" value="<?php if(!empty($suspectdetailreg)){echo $suspectdetailreg[0]['SUSPECTID'];}else{echo $mainid;} ?>" readonly>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="input-name" class="col-sm-2 control-label">Quantity</label>
                                    <div class="col-sm-5">
                                        <input type="text" class="form-control" id="input-quantity" name="input-quantity" placeholder="Quantity" value="<?php if(!empty($suspectdetailreg)) echo $suspectdetailreg[0]['QUANTITY']; ?>">
                                    </div>
                                </div>

                                <div class="form-group">
                                    <label for="select-assign-type" class="col-sm-2 control-label">UOM</label>
                                    <div class="col-sm-2">
                                        <select class="form-control" id="input-uom" name="input-uom">
                                            <?php
                                                $uom = array('Un','Au');
                                                foreach ($uom as $key) {
                                                  if (!empty($suspectdetailreg)) {
                                                    if ($suspectdetailreg[0]['UOM']==$key) {
                                ...
                                <div class="form-group">
                                    <label for="select-assign-type" class="col-sm-2 control-label">Transaction Model</label>
                                    <div class="col-sm-4">
                                        <select class="form-control" id="select-transaction-model" name="select-transaction-model">
                                            <?php
                                                $Transactionmodel = array('New','Use','Rental','Training');
                                                foreach ($Transactionmodel as $key) {
                                                  if (!empty($suspectdetailreg)) {
                                                    if ($suspectdetailreg[0]['TRANSACTION_MODEL']==$key) {
                                                      echo "<option selected='selected' value=".$key.">".$key."</option>";
                                                    }else{
                                                      echo "<option value=".$key.">".$key."</option>";
                                                    }
                                ...
                                <div class="form-group">
                                    <label for="input-name" class="col-sm-2 control-label">ODDS</label>
                                    <div class="col-sm-5">
                                        <input type="text" class="form-control" id="input-odds" name="input-odds" placeholder="ODDS" value="<?php if(!empty($suspectdetailreg)) echo $suspectdetailreg[0]['ODDS']; ?>">
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-4">
                                        <select class="form-control" id="select-lead-status" name="select-lead-status">
                                            <?php
                                                $assigntype = array('Verified','Create_Quotation');
                                                foreach ($assigntype as $key) {
                                                  if (!empty($suspectdetailreg)) {
                                                    if ($suspectdetailreg[0]['STATUS']==$key) {
                                                      echo "<option selected='selected' value=".$key.">".str_replace("_", " ", $key)."</option>";
                                                    }else{
                    ...
                    <div class="box-header with-border">
                        <h3 class="box-title">Header Suspect</h3>
                    </div>


                    <!-- for notes / header only -->
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            <input type="hidden" class="form-control" id="select-customer" name="select-customer" placeholder="Customer" value="<?php if(!empty($suspectReg[0]['CUSTOMERID'])) echo $suspectReg[0]['CUSTOMERID']; ?>" readonly>
        ...
        <script type="text/javascript">
            function goBack() {
                window.location.href = '../../suspectdetail/'+<?php echo $mainid;?>;
            }
        </script>
